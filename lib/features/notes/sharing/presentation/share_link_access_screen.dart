...
class _ShareLinkAccessScreenState extends ConsumerState<ShareLinkAccessScreen> {
  ...
  @override
  void didUpdateWidget(covariant ShareLinkAccessScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.token != widget.token) _redirectScheduled = false;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return access.when(
      ...
      loading: () => const _ShareLinkAccessShell(
        child: Center(child: CircularProgressIndicator()),
      ),
      error: (_, _) => const _ShareLinkAccessShell(
        child: AppErrorView(
          title: ShareLinkStrings.accessErrorTitle,
          subtitle: ShareLinkStrings.accessErrorSubtitle,
        ),
...
class _ShareLinkAccessShell extends StatelessWidget {
  const _ShareLinkAccessShell({required this.child});
