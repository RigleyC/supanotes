class ShareLinkStrings {
  ShareLinkStrings._();

  static const sectionTitle = 'Link público';
  static const inactiveDescription =
      'Qualquer pessoa com este link poderá ler a nota.';
  static const activate = 'Ativar link público';
  static const copy = 'Copiar';
  static const share = 'Compartilhar';
  static const replace = 'Substituir';
  static const revoke = 'Revogar';
  static const copySuccess = 'Link copiado';
  static const statusError = 'Não foi possível consultar o link público.';
  static const actionError = 'Não foi possível alterar o link público.';
  static const replaceConfirmTitle = 'Substituir link público?';
  static const replaceConfirmMessage =
      'O link atual será invalidado imediatamente.';
  static const revokeConfirmTitle = 'Revogar link público?';
  static const revokeConfirmMessage =
      'Quem tiver o link perderá o acesso imediatamente.';
  static const replaceConfirmLabel = 'Substituir';
  static const revokeConfirmLabel = 'Revogar';
  static const accessErrorTitle = 'Este link é inválido ou foi revogado.';
  static const accessErrorSubtitle = 'Peça ao proprietário uma nova permissão.';
  static const sharedNoteTitle = 'Nota compartilhada';
  static const readerErrorTitle = 'Não foi possível abrir esta nota.';
}
