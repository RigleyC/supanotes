...
class OtDocumentCodec {
  ...
  Map<String, dynamic> encodeNode(DocumentNode node) {
    ...
    if (node is TaskNode) {
      ...
      metadata['mimeType'] = node.metadata['mimeType'] ?? 'application/octet-stream';
  ...
  DocumentNode createNodeFromBlockType({
    ...
    if (type == 'bulletList') {
      return ListItemNode(
          id: nodeId, itemType: ListItemType.unordered, text: text);
    }
    if (type == 'orderedList') {
      return ListItemNode(
          id: nodeId, itemType: ListItemType.ordered, text: text);
    }
    if (type == 'task') {
  ...
  List<Map<String, dynamic>> encodeAttributedTextToDelta(AttributedText text) {
    ...
    while (pos < plainText.length) {
      ...
      while (runEnd < plainText.length) {
        ...
        for (final marker in text.spans.markers) {
          if (marker.markerType == SpanMarkerType.start && marker.offset <= runEnd) {
            final end = findSpanEnd(text.spans.markers, marker);
