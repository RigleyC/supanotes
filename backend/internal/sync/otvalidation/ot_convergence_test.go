...
func assertConvergence(t *testing.T, base, a, b ot.Delta) {
...
func TestOT_ConcurrentInsertSamePosition(t *testing.T) {
	base := ot.New(nil).Insert("Hello", nil)
	a := ot.New(nil).Retain(5, nil).Insert(" World", nil)
	b := ot.New(nil).Retain(5, nil).Insert(" Guys", nil)

	assertConvergence(t, base, a, b)
}

func TestOT_DeleteOverlappingInsert(t *testing.T) {
	base := ot.New(nil).Insert("Hello World", nil)
	a := ot.New(nil).Retain(6, nil).Delete(5)
	b := ot.New(nil).Retain(6, nil).Insert("Earth", nil)
...
func TestOT_InsertInsideDeleteRange(t *testing.T) {
	base := ot.New(nil).Insert("Hello World", nil)
	a := ot.New(nil).Retain(3, nil).Delete(5)
	b := ot.New(nil).Retain(5, nil).Insert("X", nil)
...
func TestOT_ConflictingAttributes(t *testing.T) {
	base := ot.New(nil).Insert("Hello World", nil)
	a := ot.New(nil).Retain(6, nil).Retain(5, map[string]interface{}{"bold": true})
	b := ot.New(nil).Retain(6, nil).Retain(5, map[string]interface{}{"italic": true})
...
func generateRandomDelta(docLength int, r *rand.Rand) ot.Delta {
	d := ot.New(nil)
	cursor := 0
	...
	for i := 0; i < actionsCount; i++ {
		if cursor >= docLength {
			d = d.Insert(randomString(r.Intn(5)+1, r), nil)
			break
		}
...
func TestOT_FuzzingConvergence(t *testing.T) {
	...
	for i := 0; i < iterations; i++ {
		...
		baseDoc := ot.New(nil).Insert(baseText, nil)
