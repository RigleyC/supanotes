        ...
        while (remaining > 0 && currPos.right != null) {
          if (!currPos.right!.deleted && currPos.right!.countable) {
            if (remaining < currPos.right!.length) {
              // Split the item at the insertion point.
              // getItemCleanStart returns the right-half item (starting at clock+remaining).
              final splitItem = getItemCleanStart(tr,
                  createID(currPos.right!.id.client, currPos.right!.id.clock + remaining)) as Item?;
              currPos.right = splitItem;
              currPos.left = splitItem?.left as Item?;
              currPos.index += remaining;
              remaining = 0;
              break;
            }
