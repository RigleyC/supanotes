...
class Doc extends Observable<String> implements YStructure {
  ...
  /// Unique client ID (uint32). Mutable so it can be regenerated on collision.
  int clientID;
  ...
  T? get<T extends AbstractType<dynamic>>(String name, [T Function()? typeConstructor]) {
    ...
    if (existing != null) {
      ...
      // Auto-migration: If the type was auto-registered as a YMap by the decoder,
      // and the code later requests a more specific type (like YText or YArray),
      // we migrate the internal structure (yStart, yMap, yLength) to the new constructor
      // and update the parent pointers of integrated items.
      if (existing is YMap && typeConstructor != null) {
        ...
        share[name] = newType;
        newType.integrate(this, null);

        // Update parent references of child items
        var curr = newType.yStart;
        while (curr != null) {
          curr.parent = newType;
          curr = curr.right as Item?;
        }
        for (final item in newType.yMap.values) {
          Item? currItem = item;
          while (currItem != null) {
            currItem.parent = newType;
            currItem = currItem.left as Item?;
          }
        }
        return newType as T;
      }
