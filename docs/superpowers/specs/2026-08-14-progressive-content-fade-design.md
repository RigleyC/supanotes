...
Add a shared `ProgressiveFade` widget that wraps only the content viewport. It uses `ShaderMask` with `BlendMode.dstIn`. Its default fade height is `kToolbarHeight`, so the mask follows the 56-pixel AppBar toolbar. Opacity rises through 20%, 40%, 60%, 80%, and 100% at evenly spaced stops. Calculate all stops from `bounds.height`, not the screen height, and clamp them to `0.0..1.0`.
