...
| `catalog/data/note_catalog_sync.dart` | `NoteCatalogSync`, `noteCatalogSyncProvider` | `pullRemoteNotes` hidrata o catálogo; `_pullRemoteNote` ignora notas ativas, grava snapshot e dispara `TaskProjectionEngine` para não sobrescrever edição local. |
