...
void main() {
  group('Robust Collaborative Typing & Sync Convergence Tests', () {
    ...
    test('Cadeia de operacoes sequenciais (digitacao letra por letra) de 2 usuarios simultaneos', () {
      ...
      for (int i = 0; i < word1.length; i++) {
        ...
        opsUser1.add(
          makePending(
            operationId: 'op-u1-$i',
            blockId: blockId,
            baseRevision: 0, // Todas criadas antes de receber qualquer sync remoto
            ordinal: i,
            ops: [
      ...
      for (int i = 0; i < opsUser1.length; i++) {
        ...
        final payloadMap = jsonDecode(pending.payloadJson) as Map<String, dynamic>;
      ...
      expect(textUser1, equals(textUser2), reason: 'Ambos os clientes DEVEM ter exatamente o mesmo texto final');

      // 2. Preservacao de Palavras: "GATO " e "CACHORRO " devem estar intactas e contiguas
      expect(textUser1.contains('GATO '), isTrue, reason: 'A palavra GATO  nao pode ser fragmentada ou embolada');
      expect(textUser1.contains('CACHORRO '), isTrue, reason: 'A palavra CACHORRO  nao pode ser fragmentada ou embolada');
    });

    test('Simulacao de Fuzzing Stress Test: 2 usuarios digitando frases simultaneamente com latencia', () {
      final random = Random(42); // Seed fixa para reprodutibilidade deterministica
      const blockId = 'block-fuzz';

      const user1Words = ['MACA', 'BANANA', 'LARANJA', 'UVA'];
      const user2Words = ['CARRO', 'NAVIO', 'AVIAO', 'TREM'];

      // Monta sequencias de palavras digitadas pelos usuarios
      String textWritten1 = user1Words.join(' ') + ' ';
      String textWritten2 = user2Words.join(' ') + ' ';
      ...
      for (final p in rebasedOps2) {
        docState = docState.compose(deltaFromOps(jsonDecode(p.payloadJson)['ops'] as List));
      }
      ...
      for (final w in user1Words) {
        expect(resultText.contains(w), isTrue, reason: 'Palavra "$w" do User 1 deve existir intacta na nota final');
      }
      for (final w in user2Words) {
        expect(resultText.contains(w), isTrue, reason: 'Palavra "$w" do User 2 deve existir intacta na nota final');
      }
    });
  });
}
