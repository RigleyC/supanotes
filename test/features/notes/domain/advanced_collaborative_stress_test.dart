...
void main() {
  group('Advanced Stress & Edge Cases - Sincronizacao Colaborativa', () {
    ...
    test(
      'Cenario 1: Delecao em massa (Backspace/Selecao) vs Digitacao Simultanea',
      () {
        ...
        const initialText =
            'O texto original contem palavras importantes que nao devem sumir';
        ...
        final rebasedB = rebaserB.rebase(
          pending: pendingB,
          remote: remoteA,
          finalRevision: 1,
        );
        ...
        expect(
          finalText,
          equals(
            'O texto contem MUITO palavras importantes que nao devem sumir',
          ),
        );
      },
    );
    ...
    test(
      'Cenario 2: Rebase de Fila Offline de Alta Carga (50 operacoes acumuladas)',
      () {
        final rebaserOffline = NoteOperationRebaser(
          localActorId: 'offline-user',
        );
        ...
        for (int i = 0; i < 25; i++) {
          expect(
            text.contains('R$i'),
            isTrue,
            reason: 'Operacao remota R$i deve estar presente',
          );
          expect(
            text.contains('L$i'),
            isTrue,
            reason: 'Operacao local L$i rebaseada deve estar presente',
          );
        }
      },
    );
    ...
    test(
      'Cenario 3: Monte Carlo Fuzzer de 3 Clientes Simultaneos (Invariante de Convergencia Total)',
      () {
        ...
        final clientRebasers = {
          for (final id in clientIds)
            id: NoteOperationRebaser(localActorId: id),
        };
        ...
        final clientQueues = {
          for (final id in clientIds)
            id: List<PendingNoteOperationData>.from(clientPendingOps[id]!),
        };
        ...
        while (clientQueues.values.any((q) => q.isNotEmpty)) {
          final availableClients = clientIds
              .where((id) => clientQueues[id]!.isNotEmpty)
              .toList();
          final selectedClientId =
              availableClients[random.nextInt(availableClients.length)];
        ...
        for (final clientId in clientIds) {
          ...
          final remoteForClient = serverCommittedOps
              .where((op) => op.actorId != clientId)
              .toList();
          ...
          for (final r in remoteForClient) {
            clientDoc = clientDoc.compose(
              deltaFromOps(r.payload['ops'] as List),
            );
          }
          ...
          for (final id in clientIds) {
            for (final word in clientWords[id]!) {
              expect(
                ...
                isTrue,
                reason:
                    'Palavra $word do $id deve existir sem corrupcao no documento do $clientId',
              );
            }
          }
        }
