...
void main() {
  ...
  test(
    'projects content and tasks atomically from snapshot into SQLite',
    () async {
  ...
  test(
    'rolls back note and task updates atomically when projection fails during TaskProjectionEngine execution',
    () async {
      ...
      try {
        ...
        final tasks = await (failingDb.select(
          failingDb.tasks,
        )..where((t) => t.noteId.equals(noteId) & t.deletedAt.isNull())).get();
        expect(tasks, hasLength(1));
  ...
  test('rolls back a new remote note when task projection fails', () async {
    const noteId = 'remote-atomic-note';
    final failingDb = TestFailingDatabase();
    ...
    try {
      ...
      await expectLater(
        failingDb.saveRemoteNote(
          ...
          note: NotesCompanion.insert(
            ...
            excerpt: const Value('Remote content'),
      ...
      expect(await failingDb.notesDao.getNoteById(noteId), isNull);
      expect(
        await (failingDb.select(
          failingDb.localNoteDocuments,
