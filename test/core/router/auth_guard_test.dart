...
void main() {
  group('authGuardRedirect', () {
    ...
    test('stays on the persisted route while auth is loading', () {
      const loading = AsyncValue<User?>.loading();
      // A saved note route — should NOT be interrupted during loading.
      expect(
        authGuardRedirect(
    ...
    test('redirects to /splash when on /login while loading', () {
      const loading = AsyncValue<User?>.loading();
      expect(
    ...
    test('redirects to /splash when on /register while loading', () {
      const loading = AsyncValue<User?>.loading();
      expect(
        authGuardRedirect(
    ...
    test('lets an authenticated user stay on a persisted note route', () {
      final auth = AsyncValue<User?>.data(
        const User(id: 'u-1', email: 'a@b.com', name: 'Alice'),
      );
      expect(
